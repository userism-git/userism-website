...
function initTypewriterEffect() {
  ...
  //   0 → TYPING_FRACTION            : text is typed character by character
  //   TYPING_FRACTION → +FILL_FRACTION: coloured duplicate rises from bottom
  //   remaining buffer               : fully coloured, before main-feed slides in
  const TYPING_FRACTION = 0.50;
  const FILL_FRACTION   = 0.35;

  // Read the nav colour once (set by setNavbarColorByPage before this runs)
  const navColor = getComputedStyle(document.documentElement)
                     .getPropertyValue('--nav-bg-color').trim() || '#00FFFF';
  ...
  function tick() {
    ...
    colorEl.textContent = currentText;

    // Hide blinking cursor once typing is done
    if (typeProgress >= 1) {
      output.classList.add('typing-done');
    } else {
      output.classList.remove('typing-done');
    }

    // --- Phase 2: colour fill rises from bottom via clip-path ---
    const fillStart    = TYPING_FRACTION;
    const fillEnd      = TYPING_FRACTION + FILL_FRACTION;
    const fillRaw      = (rawProgress - fillStart) / (fillEnd - fillStart);
    const fillProgress = Math.max(0, Math.min(1, fillRaw));

    // inset(top right bottom left): top decreases → reveals from bottom upward
    const topClip = 100 - fillProgress * 100;
    colorEl.style.clipPath = `inset(${topClip}% 0 0 0)`;
